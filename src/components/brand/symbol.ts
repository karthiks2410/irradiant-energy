/**
 * Geometry of the IE symbol (docs/decisions.md D-005) for the generated images: the favicon and
 * app-icon files that scripts/generate-icons.ts writes, and the next/og images that render
 * <BrandMark/>. The path data is the owner-supplied master on its 100-unit canvas, copied verbatim
 * from src/components/brand/Logo.tsx; if the master changes, update both and run `npm run icons`.
 *
 * Plain TypeScript with no imports, so Node can load it from scripts/ without a build step.
 */
...
/** The symbol's own crop of the master canvas: a 52-unit square centred on (50, 50). */
export const SYMBOL_CROP = 52;
