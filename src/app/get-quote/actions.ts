...
/* ---------------------------------------------------------------------------
   ...
   PII-free logs — with two differences: no email is collected, and the bill is a range. The
   visitor sees the estimate as ranges straight away; the sales alert carries one representative
   figure and says which one it is.
   --------------------------------------------------------------------------- */
...
export async function submitQuickQuote(_prev: QuickQuoteState, formData: FormData): Promise<QuickQuoteState> {
  try {
    ...
    return { ok: false, error: ERROR_SEND };
  }
}
...
async function handleQuickQuote(formData: FormData): Promise<QuickQuoteState> {
  ...
  if (parsed.kind === "spam") {
    logLeadEvent("warn", "lead_spam_dropped", { reference });
    // Same answer as a real submit, so a bot learns nothing; the figures are the bucket's own.
    return { ok: false, error: ERROR_SEND };
  }
  if (parsed.kind === "too-fast") {
    logLeadEvent("warn", "lead_too_fast", { reference });
    return { ok: false, error: ERROR_TOO_FAST };
  }
  if (parsed.kind === "invalid") {
    return { ok: false, error: ERROR_INVALID, fieldErrors: parsed.fieldErrors };
  }
  ...
  if (!apiKey) {
    ...
    // Vercel build, so a deployed site never takes this branch — and with no key nothing could be
    // sent anyway.
    if (process.env.NODE_ENV !== "production") {
      logLeadEvent("info", "lead_dry_run", { reference });
      return { ok: true, reference, whatsappHref: customerWhatsappHref(reference), summary: summarise(quick.segment, quick.billBucket, estimate) };
    }
    logLeadEvent("error", "lead_email_unconfigured", { reference });
    return { ok: false, error: ERROR_SEND, whatsappHref: customerWhatsappHref(reference), reference };
  }
  ...
  const context = {
    ...
    billRange: { label: labelFor(quick.segment, quick.billBucket)!, representativeBillInr: estimate.representativeBillInr },
  };
  ...
  return {
    ...
    reference,
    whatsappHref: customerWhatsappHref(reference),
    summary: summarise(quick.segment, quick.billBucket, estimate),
  };
}

/**
 * The popup's optional second step. Nothing stores the first step, so the visitor's own fields come
 * back with the reference; they are re-validated like any other input. Only the customer email and
 * a short note to sales are sent — both about this enquiry, neither marketing.
 */
export async function emailQuickQuote(_prev: QuickEmailState, formData: FormData): Promise<QuickEmailState> {
  try {
    ...
    if (!parsed.success) {
      ...
      return emailIssue
        ? { ok: false, error: ERROR_INVALID, fieldError: emailIssue.message }
        : { ok: false, error: ERROR_SEND };
    }
    ...
    const context = {
      ...
      submittedAt: new Date(),
      source: "quick quote popup" as const,
      billRange: { label: labelFor(request.segment, request.billBucket)!, representativeBillInr: estimate.representativeBillInr },
    };
    const resend = new Resend(apiKey);
