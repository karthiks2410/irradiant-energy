...
export async function generateMetadata() {
  ...
  return pageMetadata({ title: meta.title, description: meta.description, path: "/about", locale });
}
...
export default async function AboutPage() {
  ...
  const { breadcrumb, stands, mission, story, brand, facts, values, team, closingCta } = content.about;

  return (
    <>
      <BreadcrumbJsonLd items={[{ name: breadcrumb, path: "/about" }]} />
      ...
      <PageHero
        ...
        title={<AccentedTitle text={mission.title} accent={mission.accent} />}
        lead={mission.lead}
      />
      ...
      <Section surface="canvas" aria-labelledby="about-story-heading">
        <SectionHeading
          ...
          title={<AccentedTitle text={story.copy.title} accent={story.copy.accent} />}
        />
      ...
      <Section surface="dark" aria-labelledby="about-values-heading">
        <SectionHeading
          ...
          title={<AccentedTitle text={values.copy.title} tail={2} accent={values.copy.accent} />}
        />
        <Reveal className="mt-12 lg:mt-16">
          <CardGrid columns={3}>
      ...
      <Section surface="white" aria-labelledby="about-team-heading">
        ...
        <Reveal className="mt-12 lg:mt-16">
          <CardGrid columns={3}>
            {team.members.map((member) => (
              // The names stay Latin in both locales: `name` is English-owned by the overlay type
              // (FIXED_KEYS), the roster is an owner decision, and the initials are cut from the
              // same string the card prints.
        ...
        {showPlaceholders && (
          // Development scaffolding, not copy: nothing here renders in production, so it is not
          // translated (lib/env.ts `showPlaceholders`).
      ...
      <ClosingCtaBand
        ...
        whatsappText={fill(closingCta.whatsappPrompt.text, { siteName: site.name })}
      />
    </>
  );
}
